...
Используется генератором spitfire/tools/generate_spitfire.py.
"""
...
def export_glb(mesh, path, materials, name="Supermarine Spitfire"):
    ...
    gltf = {
        "asset": {"version": "2.0", "generator": "meshkit (github.com/Klischa/3D/spitfire)"},
        "scene": 0,
...
def export_stl(mesh, path, name="Spitfire"):
    with open(path, "wb") as fh:
        header = ("%s — procedural model (github.com/Klischa/3D/spitfire)" % name).encode("ascii", "replace")
